...
export class TennisFirstSetEngine implements MarketEngine {
  name = 'Tennis First Set';
  ...
  async generate(context: EngineContext): Promise<SignalCandidate[]> {
    ...
    for (const fixture of fixtures) {
      const prices = context.prices.filter((price) =>
        ...
        (price.market === 'First Set Winner' || price.market.startsWith('First Set ')) &&
        price.odds >= MIN_ODDS &&
        price.odds <= MAX_ODDS
      );
      ...
      for (const price of prices) {
        ...
        const stability = priceStability(similar.length > 1 ? similar : prices);
        const trueProbability = clamp(1 / price.odds + stability * 0.025 + 0.008, 0.01, 0.76);
        ...
        candidates.push({
          ...
          reason: 'Fast-start tendency + first-set market stability + early rhythm profile'
        });
      }
    }
